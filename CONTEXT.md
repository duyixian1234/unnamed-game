...
- **Weapon Level (武器等级)**: the upgrade stage of a WeaponKind, 1 through 8. Shared across all weapons of the same kind.
- **Weapon Upgrade Path (武器升级路线)**: the per-WeaponKind sequence of upgrades from Level 1 to 8. Levels 2–7 each offer 2 upgrade options; upgrades are free (chosen, not purchased). Options are mechanic effects only (attack speed, range, knockback, size, extra instances) — never direct damage; damage growth comes from the Shop.
- **Weapon Evolution (武器质变)**: the qualitative, behavior-changing transformation a Weapon undergoes at Level 8 (the path's final level). Not a pure stat jump — new behavior code per weapon.
...
- **Orb behavior**: each orb retains angular movement while its radius smoothly pulses from 0 to maximum and back to 0 over 1.2 seconds. Consecutive orbs alternate spin direction (clockwise / counter-clockwise) and carry distinct tints. Multiple orbs are evenly distributed in angular and radial phase; each orb explodes and respawns independently after the Bomber Orb evolution.
