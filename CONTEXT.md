...
- **Melee behavior**: base melee attacks use a 120-degree fan centered on the nearest enemy and affect all enemies inside it. Separate melee instances swing in distinct colors with staggered attack timing (never on the same frame). The Level-6 Whirlwind evolution merges every melee instance into a single blade occupying one slot, pooling their damage; the merged blade is a 360-degree continuous attack.
